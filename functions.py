...
def dt_get_data():
    ...
    SELECT
        ...
        ce.nome AS nome_categoria  -- Aqui está o nome da categoria da empresa
    FROM
        funcionario AS f
    LEFT JOIN
        cargo AS c ON f.id_cargo = c.id
    LEFT JOIN
        setor AS s ON c.id_setor = s.id
    LEFT JOIN
        empresa AS e ON s.id_empresa = e.id
    LEFT JOIN
        categoria_empresa AS ce ON e.id_categoria = ce.id
    LEFT JOIN
        localizacao AS l ON f.id_localizacao = l.id
    """
